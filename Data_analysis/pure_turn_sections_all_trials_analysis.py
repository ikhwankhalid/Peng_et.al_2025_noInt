"""
Pure Turn Sections Analysis: ALL TRIALS (No Filtering)

This script is identical to pure_turn_sections_single_loop_analysis.py
but WITHOUT the trial filtering step. This allows comparison between
filtered and unfiltered results to assess the impact of excluding
multi-loop trials.

Research Question:
    Does heading deviation vary based on erroneous integration of self-motion?
    (Comparison version: no filtering applied)
...
# Analysis parameters (IDENTICAL to single-loop version)
...
def process_all_conditions(df, conditions, speed_thresholds, min_bout_length=3,
    ...
    for condition in conditions:
        ...
        print(f"Found {len(unique_trials)} trials (ALL TRIALS - NO FILTERING)")
...
if __name__ == "__main__":
    print("="*80)
    print("PURE TURN SECTIONS ANALYSIS (ALL TRIALS - NO FILTERING)")
    print("="*80)
    print(f"\nConfiguration:")
    print(f"  Data path: {PROJECT_DATA_PATH}")
    print(f"  *** NO TRIAL FILTERING APPLIED ***")
    ...
    # Load reconstruction data (NO FILTERING STEP)
    print("\n" + "="*80)
    ...
    # Filter for useable sessions only
    ...
    print(f"Unique trials: {dfAutoPI.groupby(['session', 'trial']).ngroups}")

    # Process all conditions (NO TRIAL FILTERING)
    print("\n" + "="*80)
    print("PROCESSING ALL CONDITIONS (NO FILTERING)")
    print("="*80)
    ...
    if len(all_section_data) > 0:
        section_data_fn = os.path.join(results_dir, "pure_turn_section_data_all_trials.csv")
    ...
    if len(regression_results) > 0:
        reg_results_fn = os.path.join(results_dir, "pure_turn_section_regression_all_trials.csv")
    ...
    # Save endpoint data
    if len(all_endpoint_data) > 0:
        endpoint_data_fn = os.path.join(results_dir, "pure_turn_section_endpoints_all_trials.csv")
    ...
    if len(endpoint_regression_results) > 0:
        endpoint_reg_fn = os.path.join(results_dir, "pure_turn_section_endpoints_regression_all_trials.csv")
        endpoint_regression_results.to_csv(endpoint_reg_fn, index=False)
        print(f"Saved endpoint regression results: {endpoint_reg_fn}")

    # Print summary
    print("\n" + "="*80)
    print("SUMMARY OF RESULTS (ALL TRIALS - NO FILTERING)")
    print("="*80)

    if len(regression_results) > 0:
        print("\nSIGNED REGRESSION RESULTS (All Trials)")
        print("="*60)
    ...
    print("\n" + "="*80)
    print("ANALYSIS COMPLETE (ALL TRIALS - NO FILTERING)")
    print("="*80)
